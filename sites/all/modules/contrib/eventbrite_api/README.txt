...
admin/config/services/eventbrite/config. Next click on Request an API Key near the bottom of the
...
-------------------
From the config page, admin/config/services/eventbrite/config, once you have entered your keys,
...
admin/config/services/eventbrite/event and clicking the edit link.

Hooks
-------------------
The module currently has one hook available hook_eventbrite_api_request_alter.
This is called just before an API call is made (including pulling a request from
the cache). The $eventbrite_api object is the only variable, the parameters to
be passed through to eventbrite are available. You can also call get and set
methods for:
  op - Te operation to be performed
  op_type - the type of operation
  query_string - the query string to be passed
  server - the url of the eventbrite server
These can all be called like:
  MY_MODULE_eventbrite_api_request_alter($eventobj) {
    $op = $eventobj->get_op();
    $eventobj->set_server = 'https://updated.eventbrite.api.url';
  }
