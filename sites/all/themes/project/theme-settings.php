...
function project_form_system_theme_settings_alter(&$form, &$form_state) {
  ...
  $form['project_plugins']['gmap_api_key'] = array(
    '#type' => 'textfield',
    '#title' => t('Google Map API Key'),
    '#description' => t('When using Maplace, you need to obtain a Google Maps API key. For more info please go to !google website', array('!google' => l('Google Maps JavaScript API developers', 'https://developers.google.com/maps/documentation/javascript/get-api-key'))),
    '#default_value' => (theme_get_setting('gmap_api_key')) ? theme_get_setting('gmap_api_key') : NULL,
    '#states' => array(
      'invisible' => array(
        ':input[name=maplace]' => array('value' => 0),
      ),
    ),
  );
